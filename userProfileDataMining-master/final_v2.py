...
'''
    input_data输入数据   feature_union_lda_tv特征选择融合（LDA主题提取 tfidf_vectorize_1向量化）
    svm分类 用训练集训练数据 得到预测标签  evaluate_single计算正确率
'''

# 从整个训练集数据集中抽取部分数据作为训练模型的训练集数据和测试集数据，并且指定要使用的目标变量
def input_data(train_file, divide_number, end_number, tags):  #2
    train_words = []
    train_tags = []
    test_words = []
    test_tags = []
...
def tfidf_vectorize_1(train_words, train_tags, test_words, n_dimensionality):#5
...
def feature_union_lda_tv(train_words, test_words, train_tags, n_dimensionality, n_topics):#3
    # LDA主题提取
...
def SVM_single(train_data, test_data, train_tags):#7
    # SVM Classifier
    print('******************************SVM*****************************')
    t0 = time()
...
def evaluate_single(test_tags, test_tags_prediction):#8
    actual = test_tags
...
def feature_selection_chi2(train_data, train_tags, test_data, n_dimensionality):#6
...
def LDA(train_words, test_words, n_topics):#4
...
def test_single(tags, n_dimensionality, n_topics):#1
    train_file = 'train_data_fenci.txt'
    # 测试集起始样本位置
    divide_number = 15500#// 10
    # 测试集终止样本位置
    end_number = 17633#// 10
